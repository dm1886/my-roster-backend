    ...
    for (const day of days) {
      ...
      for (let i = 0; i < parsedDuties.length; i++) {
        ...
        for (const sector of sectors) {
          // Extract sector fields with proper mapping
          const flightNumber = sector.flightNumber || sector.flight_number || '';
          const depIATA = sector.depIATA || sector.dep_iata || sector.depIata || '';
          const arrIATA = sector.arrIATA || sector.arr_iata || sector.arrIata || '';
          ...
          if (!depIATA || depIATA.length < 3 || !arrIATA || arrIATA.length < 3) {
            requestLogger.warn({ sector }, 'Skipping sector with invalid IATA codes');
            continue;
          }
  ...
  try {
    ...
    const dayCheck = await db.query(
      `SELECT rd.id, rd.period_id
       ...
       WHERE rd.id = $1 AND rp.user_id = $2`,
      [day_id, req.user.sub]
    );
